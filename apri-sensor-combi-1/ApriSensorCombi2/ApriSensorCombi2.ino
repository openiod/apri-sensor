...
SoftwareSerial Serial1(8, 9); // serial ports RX, TX
...
void setup() {
    ...
    sessionNr = 0;
    messageNr = 0;
    subNr = 1;  // sub-number or identification of remote sensor in star-network. todo: dipswitch? 
...
//    Serial.println(serial1Available);
    ...
    if (errorCode==0) {
      char rfBuf[50];
      //sprintf(rfBuf, "sessienr;%s;%i;%i;%i;%i;%i;%i;%i;%i;%i;%i;%i;%i;%i;%i",sensorType[0],concPM1_0_CF1,concPM2_5_CF1,concPM10_0_CF1,concPM1_0_amb,concPM2_5_amb,concPM10_0_amb,rawGt0_3um,rawGt0_5um,rawGt1_0um,rawGt2_5um,rawGt5_0um,rawGt10_0um,version,errorCode);
      sprintf(rfBuf, "%s;%i;%i;%i;%i",sensorType[0],subNr,concPM1_0_CF1,concPM2_5_CF1,concPM10_0_CF1);
      if (sessionNr >0) 
        sendRfMessage(rfBuf);    
    }
...
*/
void sendRfMessage(char rfBuf[50]) {
