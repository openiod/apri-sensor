...
sensor	: {
	...
	, isSensorType: function(data) {
		//console.log('Try 1: '+ data[0]+ ' ' + this.id);
	...
	, addMeasurement: function(data) {
		...
		this.units[unitId].total.concPM1_0_CF1	+= parseFloat(data[3])/10;						
		this.units[unitId].total.concPM2_5_CF1	+= parseFloat(data[4])/10;
		this.units[unitId].total.concPM10_0_CF1	+= parseFloat(data[5])/10;
