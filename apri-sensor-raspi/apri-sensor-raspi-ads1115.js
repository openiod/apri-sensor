...
**  DS18B20, PMSA003/PMS7003, BME280, BME680, TGS5042 (ADS1115), SPS30, IPS7100, SCD30, solar (ADS1115)
**
...
if (aprisensorDevices.tgs5042!=undefined) {
  try {
    ...
    logger.info('ADS1115 module not installed (for tgs5042)');
  }
}
...
if (aprisensorDevices.solar!=undefined) {
  try {
    ...
    logger.info('ADS1115 module not installed (for solar)');
  }
}
...
var getAds1115Solar = function() {
  ...
  adc.readChannel(ADS1x15.differential.DIFF_0_1, (err, value, volts) => {
    if (err) {
      ...
      logger.info(' solar volts:', volts, ' mV');
/*
      if (counters.busy == false) {
        counters.solar.nrOfMeas++;
        counters.solar.nrOfMeasTotal++;
        counters.solar.solar += solar;
      }
*/
    }
  });
...
//        pga: ADS1x15.pga.PGA_2_048V,            // power-gain-amplifier range
        ...
        sps: ADS1x15.spsADS1115.SPS_8         // data rate (samples per second)
      });
    });
  }
}
...
//	setTimeout(processDataCycle, loopTimeCycle);
  ...
  results.solar.solar			= Math.round((counters.solar.solar/counters.solar.nrOfMeas)*100)/100;
...
//			logger.info(url);
    ...
    if (results.solar.nrOfMeas > 0) {
			redisHmsetHashAsync(timeStamp.toISOString()+':solar'
			  , 'foi', 'SCRP' + unit.id
			  , 'co', results.solar.solar
			  ).then(function(res) {
					var _res = res;
					redisSaddAsync('new', timeStamp.toISOString()+':solar')
						.then(function(res2) {
							var _res2 = res2;
						//	redisSaddAsync('solar', timeStamp.toISOString()+':solar')
			    		logger.info('solar ', timeStamp.toISOString()+':solar'+ _res2);
						});
				logger.info(timeStamp.toISOString()+':solar'+_res);
			});
		}
...
if (aprisensorDevices.solar) {
  // start processing solar sensor if available
  if (ads1115Available==true) {
    let timerIdAds1115Solar = setInterval(getAds1115Solar, 1000)
  }
}
