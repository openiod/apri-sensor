...
const uint8_t RXD2 = 16;  //D7;
// D8 werkt niet
// D7 gaat goed
// D6 gaat goed
const uint8_t TXD2 = 17;  //D8;  // not used (yet)

//#include <SoftwareSerial.h>
uint16_t Serial2Available;

//SoftwareSerial Serial2(SERPORT_RX, SERPORT_TX);
...
bool getResultsReadyPms() {
  nowTime = millis();
  if (nowTime - transactionTime > transactionTimeMax * 3) {
    //Serial.println("re-initiate Serial2 (pms)");
    //Serial2.begin(9600, SERIAL_8N1, RXD2, TXD2);
    //WiFi.begin();
    Serial.println("Restart after losing Serial2 connection (pms)");
    delay(1000);
    yield();
    //WiFi.reconnect();
    ESP.restart();
...
void readDataPms() {
  ...
  inputHigh8 = Serial2.read();
  inputLow8 = Serial2.read();
  ...
  //while (Serial2.read() != -1) {}; //clear buffer // misschien een probleem
